...
insert overwrite table data_analytics.boost_pay_acc_d partition(dt='${Y_M_D_1D}')
select 
...
from
(select * from dwd.dwd_putong_abtest_public_ab_users_a_d where dt>= '2018-05-24' and dt<='${Y_M_D_1D}' and status = 'default') t1
join
(select * from dwd.dwd_putong_abtest_public_ab_groups_a_d where dt>= '2018-05-24' and dt<='${Y_M_D_1D}'
...
on t1.ab_group_id=t2.id and t1.dt=t2.dt
left join 
(select * from dwd.dwd_daily_user_activities where dt>= '2018-05-24' and dt<='${Y_M_D_1D}') t3
on t1.user_id=t3.user_id and t1.dt=t3.dt
left join
(select * from dwd.dwd_agt_tantan_user_order_info_i_d where dt>= '2018-05-24' and dt<='${Y_M_D_1D}' 
and product_type in ('boost','boostVip','boostBadge','vip')) t4
on t1.user_id=t4.user_id and t1.dt=t4.dt
